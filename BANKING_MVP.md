...
> **Working document.** Update status as we ship. Last updated: 2026-06-18 (rate-pipeline audit + fix — migrated to single scraper-maintained row per product, retired 53 stale seed duplicates, rebuilt + promoted production snapshot `3802c441`).
...
| 2026-06-18 | **Root cause found — duplicate-ID snapshot.** The scraper switched `product_id` convention from bare (`tonik-td-6mo`) to provider-prefixed (`tonik-digital-bank:tonik-td-6mo`) in mid-May, creating PARALLEL rows in `staging.products` (68 bare seed rows frozen at May + 60 fresh prefixed rows). Both were `active_public=true`, so `build_rate_snapshot` emitted both → June 9 prod snapshot had 81 entries for ~61 products. The app only rendered correct values by luck of the `dedupeHydratedSnapshotRates` index tiebreak. | Audit triggered by user; confirmed via `staging.products` + `staging.product_snapshots`. |
| 2026-06-18 | **Fix — migrated to one scraper-maintained (prefixed) row per product.** Deactivated 53 stale bare seed duplicates; activated 38 fresh prefixed rows; kept the active GSave 2.3% row (verified live on gcash.com — real drop from 2.6%) and dropped the duplicate Netbank "existing" variant. 8 products with no fresh scrape kept on bare rows (honest-stale): `cimb-maxsave-24mo, dbp-savings, landbank-iaccess, maribank-td-3mo, maya-personal-goals, tonik-savings, tonik-group-stash, pagibig-mp2`. Backup of pre-change active flags in `staging.products_active_bak_20260618`. | CTO directive: ensure the vertical works. Rollback = restore backup + `promote_specific_snapshot('35d86a5b-…')`. |
| 2026-06-18 | Rebuilt staging + promoted production snapshot `3802c441-e514-46b3-95f5-671f80f00a17` — 66 unique products, 22 providers, zero duplicate IDs. Verified live via `/api/rates`: Tonik 6mo 4.5%, GSave 2.3%, Salmon 12mo 8.24%, Maya 6mo TD 6.0%, all re-scraped products dated 2026-06-18. | End of fix session. |
| 2026-06-18 | Added `canonicalizeProviderName()` in `lib/rates.ts` — the prefixed scraper rows shortened provider display names (`Landbank of the Philippines`→`Landbank`, `BanKo (by BPI)`→`BanKo`, `DiskarTech by RCBC`→`DiskarTech`, `Komo by EastWest Bank`→`Komo`). Canonicalizes back for consistency + parent-bank trust signal. **Requires a deploy to take effect** (data fix is already live). | Cosmetic consistency on the savings list. |

---

## Open questions / TBD

- [ ] **🔴 No automated production promotion.** The scraper refreshes `staging.products` daily, but production snapshots are only promoted manually (May 21 → June 9 → June 18). Without automation, rates re-go-stale within ~3 weeks. Decide: add build+promote to the scraper's GitHub Action, OR a daily scheduled routine, OR Supabase `pg_cron` (not currently installed). Needs a call on review-gating (auto-promote approved data vs. human gate).
- [ ] **🔴 Scraper dual-writes product IDs.** The real fix lives in `truva-scraping`: stop creating new prefixed-ID rows alongside the old bare rows, and don't re-introduce retired seed rows. Until fixed, each scrape may re-create stale duplicates in `staging.products`. The 2026-06-18 DB fix is a stopgap.
- [ ] **🟡 Maya TD override may be stale.** `lib/rates.ts` hard-pins `maya-td-3mo`=5.0% and `maya-td-12mo`=5.5% (banner-image source of truth, ~April). June scraper reads 5.5%/5.75% from Maya's HTML. If Maya raised rates, we're under-showing; if the HTML is still wrong, the override is correct. Needs a human to check Maya's in-app TD Plus banner and either remove the override or refresh the pinned values.
- [ ] **🟡 8 products have no fresh scrape coverage** and remain on May dates: `cimb-maxsave-24mo` (3/6/12mo all dropped — 24mo likely also lower), `dbp-savings`, `landbank-iaccess`, `maribank-td-3mo`, `maya-personal-goals`, `tonik-savings`, `tonik-group-stash`. Add them to the scraper's target list.
