...
const impressionSchema = z.object({
  product_id: z.string().min(1).max(120).trim(),
  provider: z.string().min(1).max(120).trim(),
  category: z.string().min(1).max(40).trim(),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { allowed, retryAfterSec } = checkRateLimit(`affiliate-impression:${ip}`, 60 * 1000, 240);
    ...
    if (error) {
      console.error('Affiliate impression insert failed', {
        code: error.code,
    ...
    console.error('Affiliate impressions route error:', error);
